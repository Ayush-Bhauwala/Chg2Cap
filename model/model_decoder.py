...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        # learnable
        # x = x + self.embedding_1D(torch.arange(52).to(device)).unsqueeze(1).repeat(1,x.size(1),  1)
...
class Mesh_TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.multihead_attn = nn.MultiheadAttention(
            int(d_model), nhead, dropout=dropout
        )
    ...
    def forward(
        ...
        enc_att, att_weight = self._mha_block(
            self_att_tgt, memory, memory_mask, memory_key_padding_mask
        )
...
class StackTransformer(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None):
        ...
        self.layers = torch.nn.modules.transformer._get_clones(
...
class DecoderTransformer(nn.Module):
    ...
    def forward(self, x1, x2, encoded_captions, caption_lengths):
        ...
        mask = torch.triu(
            torch.ones(word_length, word_length) * float("-inf"), diagonal=1
        )
        mask = mask.to(device)
        ...
        # encoded_caption = torch.cat((encoded_captions, torch.zeros([batch, 1], dtype = int).to(device)), dim=1)
    ...
    def sample(self, x1, x2, k=1):
        ...
        # Weight = torch.zeros(1, self.max_lengths, x.size(0)).to(device)
