...
def train_epoch(
    model, encoder, encoder_trans, dataloader, loss_function, optimizer, device, epoch
):
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Validate
        val_acc, val_loss = validate(model, encoder, encoder_trans, val_loader, DEVICE)
        ...
        if val_loss < best_val_loss:
            ...
            save_path = os.path.join(SAVE_DIR, f"lstm_decoder_loss_{val_loss:.4f}.pth")
            torch.save(checkpoint, save_path)
            print(f"✓ Saved best model (val loss): {save_path}\n")
        else:
            patience_counter += 1
            print(
                f"No improvement in val loss for {patience_counter} "
                f"epoch(s). Patience: {PATIENCE}"
            )
