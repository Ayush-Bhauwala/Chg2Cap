...
def main(args):
    ...
    if args.checkpoint is None:
        ...
        encoder_optimizer = (
            torch.optim.Adam(params=encoder.parameters(), lr=args.encoder_lr)
    ...
    encoder_lr_scheduler = (
        torch.optim.lr_scheduler.StepLR(encoder_optimizer, step_size=5, gamma=0.5)
        if args.fine_tune_encoder
        else None
    )
    encoder_trans_lr_scheduler = torch.optim.lr_scheduler.StepLR(
        encoder_trans_optimizer, step_size=5, gamma=0.5
    )
    decoder_lr_scheduler = torch.optim.lr_scheduler.StepLR(
        decoder_optimizer, step_size=5, gamma=0.5
    )
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for id, (imgA, imgB, _, _, token, token_len, _) in enumerate(train_loader):
            ...
            if args.grad_clip is not None:
                ...
                torch.nn.utils.clip_grad_value_(
                    encoder_trans.parameters(), args.grad_clip
                )
                if encoder_optimizer is not None:
                    torch.nn.utils.clip_grad_value_(
        ...
        if Bleu_4 > best_bleu4:
            ...
            model_name = (
                str(args.data_name)
                + "_batchsize_"
                + str(args.train_batchsize)
                + "_"
                + str(args.network)
                + "Bleu_4_"
                + str(round(10000 * best_bleu4))
                + ".pth"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Remote_Sensing_Image_Changes_to_Captions"
    )

    # Data parameters
    parser.add_argument(
        "--data_folder",
        default="/root/Data/LEVIR_CC/images",
    ...
    parser.add_argument(
        "--network", default="resnet101", help="define the encoder to extract features"
    )
    parser.add_argument(
        "--encoder_dim",
        default=2048,
        help="the dimension of extracted features using different network",
    )
    parser.add_argument(
        "--feat_size",
        default=16,
